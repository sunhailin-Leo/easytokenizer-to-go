...
void Tokenizer::wordpiece_tokenize_with_offsets(const std::string& text,
    std::vector<std::string>& tokens,
...
Tokenizer::wordpiece_tokenize(const std::string& text) const
{
  ...
  wordpiece_tokenize_with_offsets(text, tokens, offsets);
  return tokens;
}

void Tokenizer::encodeWithIds(const std::string& text,
    std::vector<SizeT>& input_ids,
    ...
    SizeT max_length) const
{
  ...
  // padding
  input_ids.resize(max_length);
  token_type_ids.resize(max_length);
...
void Tokenizer::encodeMultiThread(const std::vector<std::string>& texts,
    ...
    SizeT max_length) const
{
  ...
  if (num_threads <= 1)
    for (SizeT i = 0; i < n; i++)
      encodeWithIds(texts[i], input_ids[i], token_type_ids[i], attention_mask[i], offsets[i],
